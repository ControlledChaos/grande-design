...
// namespace Grande_Design\Includes\Tools;
...
function themedrive_add_pages() {
    add_theme_page(
        ...
        __( 'Theme Test', 'grande-design' ),
        'edit_theme_options',
        GGD_ADMIN_SLUG . '-theme-test',
...
function themedrive_switcher() {
    ...
    if ( themedrive_is_enabled() ) {
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is enabled with', 'grande-design' ),
            $theme_name
        );
    } else {
        $get_theme  = wp_get_theme();
        ...
        echo sprintf(
            ...
            __( 'Live Theme Test is disabled', 'grande-design' ),
            __( 'The active theme is', 'grande-design' ),
            $theme_name
        );
    }
...
function themedrive_options_page() {
    ...
    if ( isset( $_POST['button'] ) && 'Enable Theme Drive' == $_POST['button'] ) {
        ...
        $msg_status = sprintf(
            ...
            __( 'Live theme test is enabled with', 'grande-design' ),
            $get_theme_name,
            __( 'theme', 'grande-design' ),
            esc_url( site_url() ),
            __( 'View Site', 'grande-design' )
        );
        ...
        $msg_status = __( 'Live theme test has been disabled.', 'grande-design' );
...
?>
<div class="wrap" >
    <h2><?php _e( 'Live Theme Test', 'grande-design' ); ?></h2>
    <?php $action_url; ?>
    ...
    <?php wp_nonce_field( 'theme-drive' ); ?>
        <h2><?php _e( 'Instructions', 'grande-design' ); ?></h2>
        <ol>
            <li><?php _e( 'Select a theme to preview live on the site from the select box below (lists all installed themes).', 'grande-design' ); ?></li>
            <li><?php _e( 'Enable theme test', 'grande-design' ); ?></li>
            <li><?php _e( 'Once the theme is ready to go live, disable theme test and activate the theme on the Themes page.', 'grande-design' ); ?></li>
        </ol>
   	    <p><?php _e( 'Additionally you may add "?theme=xxx" to a URL, where xxx is the slug of the theme you want to test.', 'grande-design' ); ?></p>
        <?php themedrive_switcher(); ?>
        <h2><?php _e( 'User Role', 'grande-design' ); ?></h2>
        <p><?php _e( 'Specify the level of users to have access to the selected theme preview.', 'grande-design' ); ?></p>
        <p><?php _e( 'By default it is set to 10 (admin only). Level 7 are editors, level 4 are authors and level 1 are contributors.', 'grande-design' ); ?></p>
        <p><?php _e( 'The access level is ignored for accessing the site with ?theme=xxx paramaeter.', 'grande-design' ); ?></p>
        <p><input style="border-width: inherit; border-style: inherit; border-color: inherit; width: 30px;" name="access_level" id="access_level" value="<?php echo esc_attr( $access_level ); ?>" /> <label for="access_level"><?php _e( 'Access level', 'grande-design' ); ?></label></p>
        <p class="submit">
