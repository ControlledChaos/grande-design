...
if ( ! defined( 'DOING_AJAX' ) && ! defined( 'DOING_AUTOSAVE' ) ) {
    add_action( 'admin_init', 'ggd_check_wp_debug_define' );
}

function ggd_check_wp_debug_define() {
	if ( defined( 'WP_DEBUG' ) && ! WP_DEBUG ) {
		$dev_mode = get_option( 'ggd_site_development' );
		if ( $dev_mode ) {
			ggd_set_wp_debug_define( true );
		} elseif ( false == $dev_mode )  {
			ggd_set_wp_debug_define( false );
		}
	}
}
 ...
 * @return void
 */
function ggd_set_wp_debug_define( $turn_it_on ) {
	...
	$config_file_path = ggd_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
    }
	...
	*/
	apply_filters( 'ggd_set_wp_debug_define', $turn_it_on );
 ...
 */
function ggd_find_wpconfig_path() {
 ...
 * @return string|bool The path of wp-config.php file or false
 */
register_deactivation_hook( __FILE__, 'ggd_deactivation' );

function ggd_deactivation() {
    // set WP_DEBUG back to false
    set_ggd_wp_debug_off( false );
}

function set_ggd_wp_debug_off( $turn_it_off ) {
	// Get path of the config file
	$config_file_path = ggd_find_wpconfig_path();
    if ( ! $config_file_path ) {
		return;
	 ...
	 * @param string $turn_it_on The value of WP_DEBUG constant
	*/
	apply_filters( 'ggd_set_wp_debug_define', $turn_it_off );
