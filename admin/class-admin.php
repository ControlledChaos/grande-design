...
class Admin {
	...
	private function dependencies() {
		...
		require_once GGD_PATH . 'admin/class-settings.php';

		// Add icons to the titles of ACF tab and accordion fields, if active.
		if ( ggd_acf_pro() && ! get_option( 'ggd_acf_activate_settings_page' ) ) {
			include_once GGD_PATH . 'admin/class-acf-tab-icons.php';
		}

		// Include custom fields for Advanced Custom Fields Pro, if active.
		if ( ggd_acf_pro() ) {
			include_once GGD_PATH . 'admin/class-settings-fields-site-acf.php';
		}
		...
		if ( ( ggd_classicpress() || ggd_new_cms() ) && $editor ) {
			include_once GGD_PATH . 'admin/classic-editor/classic-editor.php';
		}

		// Functions for dasboard widgets, excluding the welcome panel.
		require_once GGD_PATH . 'admin/dashboard/class-dashboard.php';

		// Functions for admin menu item positions and visibility.
		require_once GGD_PATH . 'admin/class-admin-menu.php';

		// Add menus to the admin toolbar.
		require_once GGD_PATH . 'admin/class-admin-toolbar-menus.php';
	...
	public function enqueue_styles() {
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( GGD_ADMIN_SLUG . '-admin', GGD_URL . 'admin/assets/css/admin.min.css', [], GGD_VERSION, 'all' );
		 ...
		 * @since 1.0.0
		 */
		wp_enqueue_style( GGD_ADMIN_SLUG . '-tooltips', GGD_URL . 'admin/assets/css/tooltips.min.css', [], GGD_VERSION, 'all' );
		 ...
		 */
		if ( ggd_acf() ) {
			wp_enqueue_style( GGD_ADMIN_SLUG . '-acf', GGD_URL . 'admin/assets/css/acf.css', [], GGD_VERSION, 'all' );
		}
		...
		if ( $welcome ) {
			wp_enqueue_style( GGD_ADMIN_SLUG . '-welcome', GGD_URL . 'admin/assets/css/welcome.css', [], GGD_VERSION, 'all' );
		}
	...
	public function get_search_form() {
		...
		require GGD_PATH . 'admin/partials/searchform.php';
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( GGD_ADMIN_SLUG . '-admin', GGD_URL . 'admin/assets/js/admin.js', [ 'jquery' ], GGD_VERSION, true );

		// Enqueue Conditionalize for conditional form fields.
		wp_enqueue_script( GGD_ADMIN_SLUG . '-conditionalize', GGD_URL . 'admin/assets/js/conditionize.flexible.jquery.min.js', [ 'jquery' ], GGD_VERSION, true );
