...
class Admin_Pages {
	...
	public function __construct() {
		...
		add_action( 'admin_menu', [ $this, 'plugin_help_page' ] );

		// Add admin header.
		if ( ggd_acf_options() ) {

			// If the ACF free plus the Options Page addon or Pro plugin is active.
			$admin_header = get_field( 'ggd_use_admin_header', 'option' );
		} else {

			// Otherwise look for the WP API setting.
			$admin_header = get_option( 'ggd_use_admin_header' );
		}
	...
	public function plugin_help_page() {
		...
		if ( ggd_acf_options() ) {
			...
			// Get the field.
			$link_label = get_field( 'ggd_site_plugin_link_label', 'option' );

		// If ACF is not active, get the field from the WordPress/ClassicPress options page.
		} else {

			// Get the field.
			$link_label = sanitize_text_field( get_option( 'ggd_site_plugin_link_label' ) );
		}
		...
		// Otherwise use Site Plugin as the label.
			...
			$label = __( 'Site Plugin', 'grande-design' );
		}
		...
		if ( ggd_acf_options() ) {
			...
			// Get the field.
			$link_icon  = get_field( 'ggd_site_plugin_link_icon', 'option' );

		// If ACF is not active, get the field from the WordPress/ClassicPress options page.
		} else {

			// Get the field.
			$link_icon  = sanitize_text_field( get_option( 'ggd_site_plugin_link_icon' ) );
		}
		...
		// Otherwise use the scholar's cap icon to imply instruction.
			...
			$icon = __( 'dashicons-welcome-learn-more', 'grande-design' );
		}

		if ( true == $position ) {
			$this->plugin_help_page = add_menu_page(
				$label,
				$label,
				'manage_options',
				GGD_ADMIN_SLUG . '-page',
				[ $this, 'plugin_help_output' ],
				$icon,
				3
			);
		} else {
			$this->plugin_help_page = add_submenu_page(
				'index.php',
				$label,
				$label,
				'manage_options',
		...
		add_action( 'load-' . $this->plugin_help_page, [ $this, 'plugin_help_tab_output' ] );
	}

	/**
	 * Get output of the website help page for the plugin.
	 *
	 * @since  1.0.0
	 * @access public
	 * @return void
	 */
	public function plugin_help_output() {
		require GGD_PATH . 'admin/partials/plugin-page-help.php';
	}

	/**
	 * Add tabs to the website help page contextual help section.
	 *
	 * @since      1.0.0
	 */
	public function plugin_help_tab_output() {

		// Add to the about page.
		$screen = get_current_screen();
		if ( $screen->id != $this->plugin_help_page ) {
			return;
		}
		...
		$screen->add_help_tab( [
			'id'       => 'help_info',
			'title'    => __( 'Plugin Overview', 'grande-design' ),
			'content'  => null,
			'callback' => [ $this, 'help_info_tab' ]
		] );

		// Convert plugin tab.
		$screen->add_help_tab( [
			'id'       => 'help_pages',
			'title'    => __( 'Help Tab Content', 'grande-design' ),
			'content'  => null,
			'callback' => [ $this, 'help_pages_tab' ]
		] );

		// Add a help sidebar.
		$screen->set_help_sidebar(
			$this->plugin_help_sidebar()
		);
	 ...
	 * @since      1.0.0
	 */
	public function help_info_tab() {
		include_once GGD_PATH . 'admin/partials/help/plugin-help-info.php';
	}
	 ...
	 * @since      1.0.0
	 */
	public function help_pages_tab() {
		include_once GGD_PATH . 'admin/partials/help/plugin-help-content.php';
	}
	 ...
	 * @since      1.0.0
	 */
	public function plugin_help_sidebar() {

		$html  = sprintf( '<h4>%1s</h4>', __( 'Author Credits', 'grande-design' ) );
		$html .= sprintf(
			'<p>%s %s.</p>',
			__( 'This plugin was developed by', 'grande-design' ),
	...
	public function admin_menus() {
		...
		register_nav_menus( [
			'admin-header' => __( 'Admin Header Menu', 'grande-design' )
		] );
	}
	...
	public function admin_header() {
		...
		if ( ! empty( $admin_header ) ) {
			...
			include_once GGD_PATH . 'admin/partials/admin-header.php';
		}
	...
	public function admin_header_layout() {
		...
		$style .= '.gutenberg-editor-page .ggd-admin-header { padding-left: 20px; }';
		$style .= '.gutenberg-editor-page .ggd-admin-header:before { content: ""; display: block; width: 100%; height: 56px; }';

		// End the style block.
		$style .= '</style>';

		// Apply a filter for custom admin themeing.
		$style = apply_filters( 'ggd_admin_header_layout', $style );
	...
	public function title_placeholders( $title ) {
		...
		if ( 'post' == $screen->post_type ) {
			$post_title = esc_html__( 'Post Title', 'grande-design' );

		// Post type: page.
		} elseif ( 'page' == $screen->post_type ) {
			$post_title = esc_html__( 'Page Title', 'grande-design' );
		...
		// Post type: custom, unidentified.
			...
			$post_title = esc_html__( 'Enter Title', 'grande-design' );
		}

		// Apply a filter conditional modification.
		$title = apply_filters( 'ggd_post_title_placeholders', $post_title );
	...
	private function get_column_image( $post_ID ) {
		...
		$thumb = apply_filters( 'ggd_column_thumbnail_size', $size );
	...
	public function image_column_head( $defaults ) {
		...
		$name    = __( 'Featured Image', 'grande-design' );

		// Apply a filter for conditional modification.
		$heading = apply_filters( 'ggd_image_column_head', $name );
